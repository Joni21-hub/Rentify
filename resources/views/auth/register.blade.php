...
<body class="min-h-screen flex items-center justify-center p-4 sm:p-6 relative overflow-hidden bg-gradient-to-br from-cyan-50 via-sky-100 to-blue-200 text-slate-700">
    ...
    <div class="w-full max-w-md glass-card rounded-[2.5rem] p-8 sm:p-10 relative z-10 transition-all duration-300">
        ...
        <form action="{{ route('register') }}" method="POST" class="space-y-4">
            ...
            <!-- TOMBOL LOGIN GOOGLE -->
            <a href="{{ route('google.login') }}" class="w-full flex items-center justify-center gap-3 py-3.5 bg-white border border-slate-200 rounded-2xl shadow-sm hover:shadow-md hover:bg-slate-50 transition-all font-bold text-slate-600 text-sm mb-4">
            ...
            <div class="flex items-center gap-3 my-5">
                ...
                <span class="text-[10px] font-bold text-slate-400 tracking-wider">ATAU DAFTAR MANUAL</span>
                <div class="h-px bg-slate-200 flex-1"></div>
            </div>
            ...
            <div class="space-y-1">
                <label class="text-[11px] font-bold text-sky-800 uppercase tracking-wider block ml-1">Nomor WhatsApp Aktif</label>
                <div class="relative">
                    <span class="absolute inset-y-0 left-0 flex items-center pl-4 text-emerald-500 text-sm"><i class="fa-brands fa-whatsapp"></i></span>
                    <input type="text" name="whatsapp" value="{{ old('whatsapp') }}" required class="input-sky w-full pl-11 pr-4 py-3 rounded-2xl text-sm text-slate-800 placeholder-slate-400 focus:outline-none font-medium shadow-inner" placeholder="Contoh: 081234567890">
                </div>
            </div>

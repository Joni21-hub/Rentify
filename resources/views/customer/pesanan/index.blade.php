...
<style>
    ...
    .badge-selesai { background: #dcfce7; color: #10b981; border: 1px solid #bbf7d0; }
    .badge-batal { background: #fee2e2; color: #ef4444; border: 1px solid #fecaca; }
    ...
    .wa-link:hover { background: #10b981; color: white; }

    /* Tombol Selesaikan Pesanan */
    .btn-selesai { width: 100%; background: linear-gradient(135deg, #10b981, #059669); color: white; border: none; padding: 14px; font-size: 14px; font-weight: 800; cursor: pointer; display: flex; align-items: center; justify-content: center; gap: 8px; transition: 0.2s; }
...
<div class="history-container">
    ...
    @if(session('success'))
        <div style="background: #dcfce7; border: 1px solid #86efac; color: #065f46; padding: 14px; border-radius: 12px; margin-bottom: 20px; font-size: 13px; font-weight: 700; display: flex; align-items: center; gap: 10px;">
                    ...
                    @foreach($order->items as $item)
                        <div style="display: flex; gap: 12px; margin-bottom: 12px; border-bottom: 1px dashed #f1f5f9; padding-bottom: 12px;">
                            ...
                            <div style="flex: 1;">
                                ...
                                @endif
                                @if($item->deposit > 0)
                                    <div style="font-size: 11px; font-weight: 600; color: #b45309; margin-top: 1px;">*Deposit fisik: Rp {{ number_format($item->deposit * $item->quantity, 0, ',', '.') }}</div>
                                @endif
